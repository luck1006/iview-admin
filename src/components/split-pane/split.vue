<template>
  <div ref="outerWrapper" :class="wrapperClasses">
    <div v-if="isHorizontal" :class="[`${prefix}-horizontal`, { [`${prefix}-transition`]: transitionActive }]">
      <div :style="{right: `${anotherOffset}%`}" :class="[`${prefix}-pane`, 'left-pane', {'collapsed-wrap': leftCanCollapsed}]">
        <slot name="left"/>
        <div class="trigger-collapsed" @click="toggleLeftPaneCollapsed" v-if="leftCanCollapsed">
          <Icon :size="18" type="ios-arrow-back" :class="['icon-arrow', { 'icon-arrow-rotate': leftCollapsed }]" />
        </div>
      </div>
    ...
    <div v-else :class="[`${prefix}-vertical`, { [`${prefix}-transition`]: transitionActive }]">
...
export default {
  ...
  methods: {
    ...
    toggleLeftPaneCollapsed () {
      ...
      // let outerWidth = this.$refs.outerWrapper[this.offsetSize]
      // let value = this.valueIsPx ? this.value : (this.px2percent(outerWidth * this.value, outerWidth))
      // if (parseFloat(value) > parseFloat(this.computedMin)) {
      if (!this.leftCollapsed) {
        this.lastOffset = this.value
        this.$emit('input', this.computedMin)
        this.leftCollapsed = true
      } else {
        this.$emit('input', this.lastOffset)
        this.leftCollapsed = false
      }
    }
  },
