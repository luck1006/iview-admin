...
import { GetForeignFlightDataByMonth, GetForeignHotelDataByMonth, GetForeignTicketDataByMonth, GetForeignBookDataByMonth, shotEmail, getReceiver } from '@/api/data'
...
export default {
  ...
  methods: {
    ...
    handlelineoption (datalist) {
      ...
      var Line = ['境外机票默认推荐失败率', '境外机票验价失败率']
    ...
    handlelineoption1 (datalist) {
      ...
      var Line = ['境外机票默认推荐次数          ', '境外机票默认推荐失败次数', '境外机票验价次数', '境外机票验价失败次数', '境外机票购物车验价次数', '境外机票购物车验价失败次数', '境外机票预定页验价次数    ', '境外机票预定页验价失败次数']
    ...
    handlelineoption2 (datalist) {
      ...
      var Line = ['境外酒店默认推荐失败率', '境外酒店验价失败率']
      var color = ['#3b6291', '#943c39', '#779043', '#624c7c']
    ...
    handlelineoption3 (datalist) {
      ...
      var Line = ['境外酒店默认推荐次数         ', '境外酒店默认推荐失败次数   ', '境外酒店验价次数', '境外酒店验价失败次数', '境外酒店购物车验价次数', '境外酒店购物车验价失败次数', '境外酒店预定页验价次数     ', '境外酒店预定页验价失败次数']
      var color = ['#3b6291', '#943c39', '#779043', '#624c7c', '#388498', '#bf7334', '#3f6899', '#9c403d', '#7d9847', '#675083']
    ...
    handlelineoption4 (datalist) {
      ...
      var Line = ['境外门票默认推荐失败率', '境外门票验价失败率']
    ...
    handlelineoption5 (datalist) {
      ...
      var Line = ['境外门票默认推荐次数', '境外门票默认推荐失败次数', '境外门票验价次数', '境外门票验价失败次数', '境外门票购物车验价次数', '境外门票购物车验价失败次数', '境外门票预定页验价次数', '境外门票预定页验价失败次数']
  ...
  mounted () {
    GetForeignFlightDataByMonth().then(res => {
    ...
    GetForeignHotelDataByMonth().then(res => {
      if (res.data && res.data.ratedata && res.data.ratedata.length > 0) {
    ...
    GetForeignTicketDataByMonth().then(res => {
      if (res.data && res.data.ratedata && res.data.ratedata.length > 0) {
        console.log(res.data)
    ...
    GetForeignBookDataByMonth().then(res => {
      if (res.data && res.data.ratedata && res.data.ratedata.length > 0) {
        console.log(res.data)
